...
### Usage
NtdsAudit requires version 4.6 or newer of the .NET framework and Excel.

The script run in two parts:
1) Stage 1: Hash extraction for hashcat
	1) Put `SYSTEM` and `ntds.dit` inside the folder `input/`
	2) Install Excel
	3) Run `NtdsAudit.bat`
	4) 4 files are generated in the folder `output/`
		- `users.csv`: Raw list of users and their attributes & password
		- `hash.txt`: List of hashes for hashcat with full history for each user
		- `clear.txt`: Password stored in cleartext inside `ntds.dit`
		- `XXXX_PasswordPolicy.xlsx`: All statistics are missing at the first stage. To fill this excel, it require the stage 2.

2) Stage 2: Run hashcat with `hash.txt`

3) Stage 2: Statistics generation, require `hashcat.potfile`
	1) Put `SYSTEM`, `ntds.dit` and `hashcat.potfile` inside the folder `input/`
	2) Install Excel
	3) Run `NtdsAudit.bat`
	4) 4 files are generated in the folder `output/`
		- `users.csv`: Raw list of users and their attributes & password
		- `hash.txt`: List of hashes for hashcat with full history for each user
		- `clear.txt`: Password stored in cleartext inside `ntds.dit`
		- `XXXX_PasswordPolicy.xlsx`: Finale file with all information on all users and with statistics.
